---
title: Changelog
description: Recent changes across the Captor platform, SDK, and docs.
---

# Changelog

## Execution readiness (repository update)

- Execution-first docs and navigation, a runnable metric-based quickstart, and a fresh-process recovery demo.
- Honest boundaries for manual receipt imports, local persistence, partial-batch replay, and per-invocation budgets.
- Newsletter signup replaced by a working GitHub releases link.
- Unreleased SDK reliability fixes preserve ordinary failed receipts and expose final persistence failures. Consult the [package changelog](https://github.com/8dazo/captor/blob/main/CHANGELOG.md) before relying on these changes in an installed version.

Earlier entries below describe historical changes.
