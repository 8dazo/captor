...
const TECH = ['OpenAI-compatible APIs', 'OpenRouter', 'TypeScript SDK', 'Postgres control plane'];
    ...
    <GridSection className="bg-diagonal-lines">
      <div className="flex flex-col items-center justify-between gap-2 bg-background p-8 sm:flex-row sm:py-4">
        <BlurFade className="mb-6 sm:mb-0">
          <p className="max-w-[240px] text-center text-sm text-muted-foreground sm:text-left">
        ...
        <div className="grid grid-cols-2 gap-6 md:grid-cols-4 lg:max-w-4xl lg:gap-10">
          {TECH.map((name, index) => (
            <BlurFade
              ...
              className="flex items-center justify-center text-center text-sm font-medium text-neutral-700 dark:text-neutral-300"
            >
              {name}
            </BlurFade>
          ))}
